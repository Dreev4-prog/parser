...
def test_redis_auto_enables_even_with_stale_zero():
    out = _probe({"REDIS_URL": "redis://example.invalid:6379/0", "DISTRIBUTED_WORKERS": "0", "RAILWAY_PROJECT_ID": "p", "STABLE_SINGLE_SERVICE_MODE": "0"})
    assert out == "1 redis-auto 0"

def test_railway_without_redis_is_configuration_error():
    out = _probe({"REDIS_URL": "", "DISTRIBUTED_WORKERS": "0", "RAILWAY_PROJECT_ID": "p", "RAILWAY_REQUIRES_REDIS": "1", "STABLE_SINGLE_SERVICE_MODE": "0"})
    assert out == "0 no-redis 1"

def test_force_local_is_explicit_escape_hatch():
    out = _probe({"REDIS_URL": "redis://example.invalid:6379/0", "FORCE_LOCAL_MODE": "1", "RAILWAY_PROJECT_ID": "", "STABLE_SINGLE_SERVICE_MODE": "0"})
    assert out == "0 forced-local 0"

def test_bot_distributed_mode_disables_inline_views_and_local_workers():
    assert "if DISTRIBUTED_WORKERS or STABLE_SINGLE_SERVICE_MODE:\n    PRIMARY_SCAN_INLINE_VIEWS = False" in BOT
...
def test_stable_reset_default_ignores_redis_for_scan_execution():
    out = _probe({"REDIS_URL": "redis://example.invalid:6379/0", "RAILWAY_PROJECT_ID": "p"})
    assert out == "0 stable-single-service 0"
