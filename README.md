...
This maintenance release makes a clean checkout reproducible: safe environment
defaults are documented, generated bytecode is no longer tracked, historical
deployment notes are archived, the full test suite matches the current product,
and GitHub CI checks parsing, runtime globals, release contracts and tests.

The parser, Radar Score/HOT thresholds, database schema and stored observations
are unchanged. See `RELEASE_4_23_15.md`, `docs/ARCHITECTURE.md` and
`docs/DEPLOYMENT.md`.
